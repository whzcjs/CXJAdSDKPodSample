...
/**
 * @brief 跳过时长
 * @discussion 开屏广告跳过时长，默认5s（备注：当物料内容为视频时，跳过时长为固定的5s，不可配置）
...
/**
 * @brief 视频是否已预加载
 * @discussion 可根据该字段获取视频预加载状态
 */
@property (nonatomic, assign) BOOL materialMetaIsFromPreload;

/**
 * @brief 是否为视频物料
 * @discussion 可根据该字段判断物料返回内容
 */
@property (nonatomic, assign) BOOL materialMetaIsVideo;

/**
 * @brief 隐藏`已预加载`文案，默认为NO
 * @discussion 可根据该字段隐藏`已预加载`文案
 */
@property (nonatomic, assign) BOOL hidePreloadLabel;
