...
typedef void (^JADCompletionHandler)(BOOL success, NSError *error);
 ...
 * @brief 同步初始化方法
 * @param appID 京东广告应用标识
 * @param completionHandler 主线程回调初始化状态
 */
+ (void)initWithAppID:(NSString *_Nullable)appID syncCompletionHandler:(JADCompletionHandler)completionHandler;
 ...
 * @param completionHandler 主线程回调初始化状态
 */
+ (void)initWithAppID:(NSString *_Nullable)appID asyncCompletionHandler:(JADCompletionHandler)completionHandler;
